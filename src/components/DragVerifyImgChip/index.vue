  ...
  <div class="drag-verify-container">
    <div :style="dragVerifyImgStyle">
      ...
      <div class="refresh" v-if="showRefresh && !isPassing">
        <i-ep-RefreshLeft class="refresh-color" @click="refreshimg" />
      </div>
    ...
    >
      ...
      <div
        ...
        :style="handlerStyle"
      >
        <component :is="slotIcon" :style="componentStyle" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function () {
    ...
    // 去除引入的icon组件的reactive化较少ui变动带来的开销
    const _handlerIcon = shallowRef(this.handlerIcon);
    this.slotIcon = _handlerIcon;
  ...
  data() {
    return {
      ...
      clipBarx: 0,
      showErrorTip: false,
      slotIcon: shallowRef(this.handlerIcon),
      componentStyle: {}
    };
  },
  methods: {
    ...
    passVerify: function () {
      ...
      this.isMoving = false;
      // 去除引入的icon组件的reactive化较少ui变动带来的开销
      const _successIcon = shallowRef(this.successIcon);
      this.slotIcon = _successIcon;
      this.componentStyle = {
        color: '#76c61d'
      };
      // handler.children[0].className.baseVal = this.successIcon;
